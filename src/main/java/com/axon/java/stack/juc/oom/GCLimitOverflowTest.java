 ...
 * <p>
 * gc 频繁回收超过了限制
 * Exception in thread "main" java.lang.OutOfMemoryError: GC回收参数设置 overhead limit exceeded
 *
 *
 *  如何去解决？
 *
 *  当你遇到 JVM 中 GC回收参数设置 回收频繁、性能下降甚至出现 OutOfMemoryError 的问题时，排查和解决这个问题通常涉及以下几个步骤。你需要分析 GC回收参数设置 日志、了解内存使用情况、调整 GC回收参数设置 配置，并可能需要优化代码逻辑。下面是详细的排查步骤：
 *
 * 1. 查看 GC回收参数设置 日志
 *
 * 首先，开启并分析 GC回收参数设置 日志，这可以帮助你了解垃圾回收的频率、耗时和发生的区域。使用以下 JVM 参数启用 GC回收参数设置 日志：
 * -XX:+PrintGCDetails -XX:+PrintGCDateStamps -Xloggc:/path/to/gc.log
 *
 * 	•	每次 GC回收参数设置 发生的时间。
 * 	•	GC回收参数设置 类型（如 Minor GC回收参数设置、Major GC回收参数设置、Full GC回收参数设置）。
 * 	•	回收了多少内存以及剩余内存情况。
 * 	•	每次 GC回收参数设置 花费的时间。
 *
 * 分析点：
 *
 * 	•	Minor GC回收参数设置：频繁的 Minor GC回收参数设置 通常意味着新生代内存不足。查看堆中 Eden 区域和 Survivor 区域的大小设置。
 * 	•	Full GC回收参数设置：如果 Full GC回收参数设置 频繁发生，通常意味着老年代内存不足或者内存碎片化严重，需特别关注堆的总容量和老年代容量。
 *
 *
 * 2. 使用工具监控内存使用
 *
 ...
 *
 * 	•	VisualVM：自带内存快照分析，展示堆内存、线程、GC回收参数设置 等详细信息。
 * 	•	JConsole：实时查看堆内存使用、线程和 GC回收参数设置 相关的指标。
 ...
 * 	•	观察 Eden 区是否被过多的小对象填满，导致频繁的 Minor GC回收参数设置。
 *
 *
 * 3. 检查并调整 JVM 内存设置
 *
 ...
 * 	优化点：
 *
 * 	•	如果 Minor GC回收参数设置 频繁，可能需要增大新生代的大小。
 * 	•	如果 Full GC回收参数设置 频繁，可能需要增大老年代的大小。
 *
 * 	4. 调整 GC回收参数设置 策略
 *
 * 不同的 GC回收参数设置 收集器适用于不同的应用场景。你可以根据系统的吞吐量、延迟需求选择合适的 GC回收参数设置 策略。
 *
 * 	•	Parallel GC回收参数设置（默认）：适合吞吐量优先的场景。适用于 CPU 核心较多、对暂停时间不敏感的场景。
 * 	•	CMS（Concurrent Mark-Sweep）：适合对响应时间要求较高的应用，减少 Full GC回收参数设置 产生的长暂停时间。适合老年代较大的场景。
 * 	•	G1 GC回收参数设置：适合堆较大，追求低延迟的应用（JDK 9 以后是默认垃圾收集器）。它能够根据需要调整不同代之间的回收。
 *
 * 	    -XX:+UseG1GC  或者  -XX:+UseConcMarkSweepGC
 *
 * 5. 减少对象创建频率
 *
 * 如果内存频繁被填满，检查代码是否创建了过多的短生命周期对象。这些对象很快会被垃圾收集器回收，造成频繁的 Minor GC回收参数设置。
 *
 * 	•	尽量复用对象，避免重复创建不必要的对象。
 ...
 *6. 监控和优化代码中的大对象
 *
 * 大对象（尤其是数组、文件缓存、图片等）直接进入老年代，如果频繁创建大对象会导致老年代很快填满，引发 Full GC回收参数设置。你可以：
 *
 * 	•	优化代码逻辑，避免大对象频繁创建。
 ...
 * 7. 内存泄漏排查
 *
 * 内存泄漏会导致对象无法被 GC回收参数设置 回收，逐渐填满堆内存。排查内存泄漏的方式：
 *
 ...
 * 总结
 *
 * 通过分析 GC回收参数设置 日志、调整 JVM 参数、优化代码逻辑和选择合适的 GC回收参数设置 策略，可以有效解决 GC回收参数设置 回收频繁、超出限制的问题。以下是一些主要的方向：
 *
 * 	1.	GC回收参数设置 日志分析：确认 GC回收参数设置 的类型、频率、耗时。
 * 	2.	工具监控：使用 VisualVM 等工具实时查看内存占用情况。
 * 	3.	调整 JVM 配置：根据内存使用情况调整堆大小、GC回收参数设置 策略。
 ...
 *
 *  jcmd <pid> GC回收参数设置.heap_dump /path/to/heapdump.hprof
 *
 *
 *  5. 使用 VisualVM 工具
 *
