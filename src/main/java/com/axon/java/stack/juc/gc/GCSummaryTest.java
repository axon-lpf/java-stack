 ...
 * GC 的总结
 *
 * 	串行垃圾收集器 (Serial GC)：适用于单线程、小内存环境，简单但暂停时间长。
 * 	并行垃圾收集器 (Parallel GC)：适用于多核、高吞吐量场景，暂停时间较长但整体效率高。
 * 	并发标记清除垃圾收集器 (CMS GC)：适用于对响应时间敏感的场景，低停顿但可能存在内存碎片。
 * 	G1垃圾收集器 (G1 GC)：适用于大内存环境下对响应时间有较高要求的场景，具有可预测的低停顿时间
 *
 * 配置命令：
