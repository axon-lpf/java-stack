 ...
 * 工作原理：
 * •	串行垃圾收集器在进行垃圾收集时会暂停所有应用线程，并且使用一个单独的线程进行垃圾回收。包括年轻代的 Minor GC回收参数设置 和老年代的 Full GC回收参数设置。
 * 使用场景：
 * •	适用于单核处理器或小型应用程序，内存占用较少且对应用响应时间要求不高的环境。
 *
 * 年轻代和老年代的组合方式：
 *      年轻代：串行垃圾回收器 (Serial Young GC回收参数设置)，使用单线程进行垃圾回收。
 * 	•	老年代：串行垃圾回收器 (Serial Old GC回收参数设置)，同样使用单线程进行垃圾回收。
 * 	•	组合方式：Serial Young GC回收参数设置 + Serial Old GC回收参数设置
