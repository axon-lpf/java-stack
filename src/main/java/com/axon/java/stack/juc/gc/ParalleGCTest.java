...
/**
 2. 并行垃圾收集器 (Parallel GC回收参数设置)
 优点：
 •	多线程并行收集：在垃圾回收期间同时利用多个线程进行垃圾收集，适合多核处理器，提升了垃圾回收的吞吐量。
 •	高吞吐量：并行收集器专注于提高应用程序的吞吐量，适合长时间运行的后台计算型任务。
 缺点：
 •	较长的停顿时间：虽然是多线程回收，但同样会发生全局暂停，可能会导致较长的 GC回收参数设置 暂停时间。
 工作原理：
 •	并行垃圾收集器的年轻代和老年代垃圾收集都可以通过多个线程并行执行。JVM 会根据 CPU 核心数动态调整并行线程的数量，最大限度地利用 CPU 资源来减少垃圾回收时间。

 使用场景：
 •	适用于对吞吐量要求高的应用程序，如批处理任务、大规模数据处理应用程序，内存充足且 GC回收参数设置 暂停时间不敏感的场景。

 * 年轻代和老年代的组合方式：
 *   	年轻代：并行垃圾回收器 (Parallel Young GC回收参数设置)，使用多线程并行处理年轻代垃圾回收。
 * 	•	老年代：并行老年代垃圾回收器 (Parallel Old GC回收参数设置)，使用多线程并行处理老年代垃圾回收。
 * 	•	组合方式：Parallel Young GC回收参数设置 + Parallel Old GC回收参数设置
 ...
 *    使用以上命令后,年轻代则是用的是 Parallel Scavenge GC回收参数设置
 *  Heap
