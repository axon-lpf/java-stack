 ...
 *
 * 工作原理：
 * 	•	G1 将堆内存分成若干个大小相等的区域 (Region)，这些区域可以被用作年轻代、老年代或作为 GC回收参数设置 的其他用途。G1 使用“暂停时间模型”来控制垃圾回收的停顿时间，并通过“全局标记-清除”和“局部复制”相结合的方式进行垃圾收集。G1 会先标记对象，然后根据应用程序的内存使用模式来选择最合适的回收区域，优先回收垃圾最多的区域。
 * 	    Region 的空间大小一般是1-32M
 * 使用场景：
 * 	•	适用于对响应时间要求高的大型内存应用程序，如大数据处理、大型企业级应用等。G1 GC回收参数设置 提供了在低延迟和高吞吐量之间的平衡，特别适用于堆内存较大的应用场景。
 *
 * 年轻代和老年代的组合方式：
