 ...
 * 优点：
 * •	低停顿时间：CMS 是一种低延迟的垃圾收集器，适用于对响应时间要求高的应用程序，通过并发方式减少 GC 暂停时间。
 * •	并发标记和清除：年轻代的回收采用多线程并行处理，老年代使用并发标记-清除算法，减少了 Full GC 的停顿时间。
 * 缺点：
 * •	内存碎片：CMS 采用“标记-清除”算法，可能会导致内存碎片问题，从而影响性能。
 * •	CPU 开销大：CMS 需要占用更多的 CPU 资源，以并发方式执行垃圾收集，可能导致应用程序和 GC 之间的 CPU 竞争。
 * 工作原理：
 ...
 * 	•	老年代：CMS垃圾回收器 (Concurrent Mark-Sweep GC)，通过并发标记清除算法处理老年代垃圾回收，旨在减少老年代的停顿时间。
 * 	•	组合方式：Parallel Young GC + CMS GC
